...
module.exports = function() {
		...
		if (true){
			db.serialize(function() {
				...
				db.run("CREATE TABLE probes ("
						...
						+ "max_w REAL"
						+ ")");
				var stmt = db.prepare("INSERT INTO probes(code, system_id, min_e, min_w, max_e, max_w) VALUES (?, ?, ?, ?, ?, ?)");
				stmt.run("TEMP", "1W|28-0000054e5ffe", 23, 24.5, 30, 29);
				stmt.finalize();
				
				stmt = db.prepare("INSERT INTO probes(code, system_id, min_e, min_w, max_e, max_w) VALUES (?, ?, ?, ?, ?, ?)");
				stmt.run("PH", "Atlas|0:0", 7, 8, 10, 9);
				stmt.finalize();
	...
	Dao.prototype.getLastMeasurement = function(probeId, callback) {
		db.serialize(function() {
