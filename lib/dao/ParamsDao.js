...
module.exports = function() {
	...
	ParamsDao.prototype.getAndIncreaseTick = function(callback) {
		db.serialize(function() {
			db.get("SELECT value_int FROM params WHERE key = ?", ParamsDao.prototype.PARAMS.SYSTEM_TICKS, function(err,
					row) {
				if (err){
					throw err;
				}
				...
				db.serialize(function() {
					var stmt = db.prepare("UPDATE params SET value_int = ? WHERE key = ?");
					stmt.run(tick, ParamsDao.prototype.PARAMS.SYSTEM_TICKS, function(err) {
						if (err) {
								throw err;
							}
					});
